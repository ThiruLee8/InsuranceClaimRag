...
@router.post("/chat/retrieve", response_model=ApiResponse[RetrieveResponse])
async def chat_retrieve(
    payload: RetrieveRequest,
    service: ConversationService = Depends(get_conversation_service),
):
    """Retrieve-only (no LLM answer) for eval and search tuning."""
    try:
        result = await service.retrieve(payload)
        return ApiResponse(data=result)
    except Exception as exc:  # noqa: BLE001
        raise_http(f"Retrieve failed: {exc}", "RETRIEVE_ERROR", 500)
